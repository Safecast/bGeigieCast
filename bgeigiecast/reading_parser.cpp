...
struct _PDCLIB_status_t {
  ...
  int n;      /* print: maximum characters to be written (snprintf) */
...
const char* _PDCLIB_scan(const char* spec, struct _PDCLIB_status_t* status) {
  ...
  */
  ...
  status->width = (unsigned) strtol(spec, (char**) &spec, 10);
  if(spec == prev_spec) {
  ...
  if(status->base != -1) {
    ...
    while((status->current < status->width) &&
      ...
      if(isspace(rc)) {
        ...
        char* digitptr = static_cast<char*>(nmemchr(_PDCLIB_digits, tolower(rc), status->base));
        if(rc != '.') {
          if(digitptr == nullptr) {
            /* end of input item */
            UNGET(rc, status);
    ...
    if(!(status->flags & E_suppressed)) {
      switch(status->flags & (E_char | E_short | E_long | E_llong |
        ...
        default:
          ...
          return nullptr; /* behaviour unspecified */
      }
      ++(status->n);
    }
    return ++spec;
...
int nsscanf(const char* s, const char* format, ...) {
  ...
  while(*format != '\0') {
    ...
    if((*format != '%') || ((rc = _PDCLIB_scan(format, &status)) == format)) {
      ...
      /* nullptr return code indicates error */
      if(rc == nullptr) {
        if((*status.s == '\n') && (status.n == 0)) {
          status.n = EOF;
        }
